...
def plot_training_history(history, model_name="Model", save_plots_path=SAVE_PLOTS_PATH):
    ...
    axes[0].set_title(
        f"{model_name} - Training History", fontsize=14, fontweight="bold"
    )
    axes[0].legend()
    ...
    axes[1].set_title(
        f"{model_name} - Accuracy Progress", fontsize=14, fontweight="bold"
    )
    axes[1].legend()
...
def visualize_predictions(
    ...
    plt.suptitle(
        f"Model Predictions on Sample Images", fontsize=16, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    ...
    batch_accuracy = 100.0 * correct / num_samples
...
def plot_confusion_matrix(
    y_true, y_pred, class_names, normalize=True, save_plots_path=SAVE_PLOTS_PATH
):
...
def plot_sample_images(
    ...
    fig, axes = plt.subplots(
        num_classes, samples_per_class, figsize=(3 * samples_per_class, 3 * num_classes)
    )
    ...
    plt.suptitle(
        f"EuroSat Dataset Samples ({num_classes} classes)",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
...
def plot_model_architecture(
    model, input_size=(3, 64, 64), save_plots_path=SAVE_PLOTS_PATH
):
...
def save_training_metrics_to_csv(
    history, model_name="Model", save_plots_path=SAVE_PLOTS_PATH
):
...
def create_training_report(history, model_name, class_names, save_plots_path):
    ...
    with open(report_filename, "w") as f:
        ...
        for epoch in range(len(history["train_loss"])):
            f.write(
                f"{epoch+1:5d} | {history['train_loss'][epoch]:10.4f} | "
