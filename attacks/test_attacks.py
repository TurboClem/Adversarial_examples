...
def load_model_and_data(
    ...
    # Test dataset
    dataset = EuroSatDataset(root_dir=data_path, transform=None, train=False)
...
def demo_single_attack():
    ...
    for i in range(len(images)):
        ...
        print(
            f"  Clean pred: {class_names[preds_clean[i]]} {'✓' if preds_clean[i] == labels[i] else '✗'}"
        )
        print(
            f"  FGSM pred: {class_names[preds_fgsm[i]]} {'✓' if preds_fgsm[i] == labels[i] else '✗'}"
        )
        print(
            f"  PGD pred: {class_names[preds_pgd[i]]} {'✓' if preds_pgd[i] == labels[i] else '✗'}"
        )
        print()
...
def evaluate_all_attacks(model_type, model_path, data_path):
    ...
    model, test_loader, device, class_names = load_model_and_data(
        model_type, model_path, data_path
    )
...
def compare_models(model_path, data_path):
    ...
    for model_name in ["simple_cnn", "resnet18"]:
        ...
        model, test_loader, device, _ = load_model_and_data(
            model_name, model_path, data_path
        )
    ...
    for res in results:
        print(
            f"{res['Model']:12} | Clean: {res['Clean Accuracy']:10} | Adv: {res['Adversarial Accuracy']:10} | Success: {res['Success Rate']}"
        )
...
if __name__ == "__main__":
    ...
    compare_models(
        model_path="outputs/models/resnet18_final.pth", data_path="./data/EuroSAT_RGB"
    )
