...
class ModelTrainer:
    ...
    def train_epoch(self, train_loader, criterion, optimizer, madry):
        ...
        for batch_idx, (inputs, targets) in enumerate(pbar):
            ...
            if madry:
                ...
                alpha = madry["alpha"]
                epsilon = (
                    epsilon.to(self.device) if torch.is_tensor(epsilon) else epsilon
                )
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            train_loss, train_acc = self.train_epoch(
                train_loader, criterion, optimizer, madry
            )
            ...
            if val_acc > best_val_acc:
                ...
                epochs_no_improve = 0
                self.save_model(
                    filename="best_model.pth", save_model_path=save_model_path
                )
    ...
    def evaluate(self, test_loader):
        ...
        accuracy = (
            100.0
            * np.sum(np.array(all_preds) == np.array(all_labels))
            / len(all_labels)
        )
        return accuracy, all_preds, all_labels

    def pgd_attack_train(self, images, labels, epsilon_pixel, alpha_pixel, iterations):
        self.model.eval()
...
def create_data_loaders(
    data_path=DATA_PATH_TRAIN, batch_size=BATCH_SIZE, mode="train"  # "train" or "eval"
):
    """
